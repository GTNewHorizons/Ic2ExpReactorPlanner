...
public class AutomationSimulator extends SwingWorker<Void, String> {
    ...
    public AutomationSimulator(
            ...
            final JPanel[][] reactorButtonPanels,
            final File csvFile,
            final int csvLimit) {
        ...
        try {
            ...
            do {
                ...
                handleBrokenComponents(
                        reactorTicks, totalHeatOutput, totalRodCount, totalEUoutput, minReactorHeat, maxReactorHeat);
                ...
                if (prevReactorHeat == 0.0 && prevTotalComponentHeat == 0.0) {
                    ...
                    if (reactor.getCurrentHeat() < reactor.getMaxHeat()) {
                        if (reactor.getCurrentHeat() == 0.0) {
                            ...
                            publish(formatI18n(
                                    "Simulation.ReactorResidualHeat", reactor.getCurrentHeat(), reactorCooldownTime));
                        }
                ...
                for (int col = 0; col < 9; col++) {
                    ...
                    if (component != null) {
                        if (component.getVentCoolingCapacity() > 0) {
                            component.info.append(formatI18n(
                            ...
                            component.info.append(
                                    formatI18n("ComponentInfo.ReceivedHeat", component.getBestCellCooling()));
                            totalCellCooling += component.getBestCellCooling();
                        } else if (component.getBestCondensatorCooling() > 0) {
                            component.info.append(
                                    formatI18n("ComponentInfo.ReceivedHeat", component.getBestCondensatorCooling()));
                            ...
                            component.info.append(formatI18n(
                                    ...
                                    component.getCurrentDamage() > component.getMaxDamage()
                                            ? (int) component.getMaxDamage()
                                            : (int) component.getCurrentDamage(),
                                    (int) component.getMaxDamage()));
                        }
                        if (component.getMaxReachedHeat() > 0) {
                            component.info.append(formatI18n(
                                    "ComponentInfo.ReachedHeat",
                                    component.getMaxReachedHeat(),
            ...
            //            }
            showHeatingCooling(
                    reactorTicks); // Call to show this info in case it hasn't already been shown, such as for an
            // automated reactor.
                ...
                if (component != null && component.isBroken() && !alreadyBroken[row][col]) {
                    ...
                    if (component.getRodCount() == 0) {
                        ...
                        if (componentsIntact) {
                            ...
                            publish(formatI18n(
                                    "Simulation.FirstComponentBrokenDetails",
                                    component.toString(),
                                    row,
                                    col,
                                    reactorTicks));
                        ...
                        publish(formatI18n(
                                "Simulation.FirstRodDepletedDetails", component.toString(), row, col, reactorTicks));
                        if (reactor.isFluid()) {
        ...
        if (!showHeatingCoolingCalled) {
            ...
            if (reactorTicks >= 40) {
                ...
                if (totalHullCoolingCapacity > 0) {
                    publish(formatI18n(
                            "Simulation.HullCooling",
                            totalHullCooling / (reactorTicks - 20),
                            totalHullCoolingCapacity));
                }
                if (totalVentCoolingCapacity > 0) {
                    publish(formatI18n(
                            "Simulation.VentCooling",
                            totalVentCooling / (reactorTicks - 20),
                            totalVentCoolingCapacity));
                }
            }
        }
